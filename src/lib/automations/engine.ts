...
import { appointmentRange } from '@/lib/appointments/validation';
...
export interface AutomationContext {
  ...
  /** Appointment context exposed to appointment lifecycle automations. */
  appointment?: import('@/types').AppointmentContext;
}
    ...
    case 'create_appointment': {
      ...
      const [{ data: service }, { data: staff }] = await Promise.all([
        ...
        db
          ...
          .select('*')
          .eq('id', cfg.staff_profile_id)
          .eq('account_id', args.automation.account_id)
          .eq('is_bookable', true)
          .maybeSingle(),
      ]);
      if (!service || !staff)
        throw new Error('appointment service or staff unavailable');
      const range = appointmentRange(startsAt, service.duration_minutes);
      if (!range) throw new Error('create_appointment has invalid starts_at');
      const { data: appointment, error } = await db
        ...
        .insert({
          ...
          timezone: cfg.timezone || staff.timezone,
          status: cfg.confirm_immediately ? 'confirmed' : 'pending',
          source: 'automation',
          created_by_user_id: args.automation.user_id,
          confirmed_at: cfg.confirm_immediately
        ...
        .single();
      if (error?.code === '23P01')
        throw new Error('appointment time is no longer available');
      ...
      args.context.appointment = {
        ...
        staff_name: staff.user_id,
      };
      await db.from('appointment_events').insert({
        account_id: args.automation.account_id,
        appointment_id: appointment.id,
        event_type: cfg.confirm_immediately ? 'confirmed' : 'created',
        payload: { source: 'automation', automation_id: args.automation.id },
      });
      return `appointment ${appointment.id} created`;
    }

    case 'confirm_appointment':
    case 'cancel_appointment':
    case 'reschedule_appointment': {
      const cfg = step.step_config as AppointmentMutationStepConfig;
      const appointmentId = cfg.appointment_id
        ? interpolate(cfg.appointment_id, args)
        : args.context.appointment?.id;
      if (!appointmentId)
        throw new Error(`${step.step_type} needs appointment context`);
      const { data: current } = await db
        .from('appointments')
        .select('*, service:appointment_services(duration_minutes)')
        .eq('id', appointmentId)
        .eq('account_id', args.automation.account_id)
        .maybeSingle();
      ...
      if (step.step_type === 'confirm_appointment') {
        update.status = 'confirmed';
        update.confirmed_at = new Date().toISOString();
      } else if (step.step_type === 'cancel_appointment') {
        update.status = 'cancelled';
        update.cancelled_at = new Date().toISOString();
        update.cancellation_reason = 'Cancelled by automation';
      } else {
        const range = appointmentRange(
          interpolate(cfg.starts_at ?? '', args),
          current.service.duration_minutes
        );
        if (!range)
          throw new Error('reschedule_appointment has invalid starts_at');
      ...
      return `${step.step_type} completed`;
    }

    case 'offer_appointment_slots': {
      const cfg = step.step_config as OfferAppointmentSlotsStepConfig;
      if (!args.contactId)
        throw new Error('offer_appointment_slots needs a contact');
      const date = interpolate(cfg.date, args);
      if (!/^\d{4}-\d{2}-\d{2}$/.test(date))
        throw new Error('slot date must be YYYY-MM-DD');
      const { data: service } = await db
        ...
        .eq('is_active', true)
        .maybeSingle();
      if (!service) throw new Error('appointment service unavailable');
      let staffQuery = db
        .from('staff_scheduling_profiles')
        .select('*')
        .eq('account_id', args.automation.account_id)
        .eq('is_bookable', true);
      if (cfg.staff_profile_id)
        staffQuery = staffQuery.eq('id', cfg.staff_profile_id);
      ...
      for (const staff of staffRows ?? []) {
        const timezone = cfg.timezone || staff.timezone;
        ...
        const [{ data: windows }, { data: busy }, { data: exceptions }] =
          await Promise.all([
            db
        ...
        const extraWindows = (exceptions ?? [])
          ...
          .map((exception) => ({
            start_time: localClock(exception.starts_at, timezone),
            end_time: localClock(exception.ends_at, timezone),
          }));
      ...
      choices.sort((a, b) => a.starts_at.localeCompare(b.starts_at));
      const selected = choices.slice(
        0,
        Math.min(10, Math.max(1, cfg.limit ?? 10))
      );
      if (!selected.length) throw new Error('no appointment slots available');
      const conversationId = await resolveConversationId(args);
      const rows = selected.map((slot) => ({
        id: createAppointmentActionToken({
          accountId: args.automation.account_id,
          contactId: args.contactId!,
          revision: 0,
          action: 'book',
          ...
          staffProfileId: slot.staff_profile_id,
          timezone: slot.timezone,
        }),
        title: slot.display_label.slice(0, 24),
        description: service.name.slice(0, 72),
      }));
      const { whatsapp_message_id } = await engineSendInteractive({
        accountId: args.automation.account_id,
        userId: args.automation.user_id,
        ...
        payload: {
          kind: 'list',
          body: `Choose a time for ${service.name}`,
          button_label: 'View times',
          sections: [{ title: date, rows }],
        },
      });
      return `offered ${rows.length} slots (${whatsapp_message_id})`;
    }

    case 'send_appointment_template': {
      const cfg = step.step_config as SendAppointmentTemplateStepConfig;
      if (!args.contactId || !cfg.template_name) {
        throw new Error('send_appointment_template needs contact + template');
      }
      const conversationId = await resolveConversationId(args);
      const variables = cfg.variables ?? {};
      const params = Object.keys(variables)
        .sort((a, b) => Number(a) - Number(b))
        .map((key) => interpolate(variables[key], args));
      const { whatsapp_message_id } = await engineSendTemplate({
        accountId: args.automation.account_id,
        userId: args.automation.user_id,
        conversationId,
      ...
      return `appointment template sent (${whatsapp_message_id})`;
    }
...
function interpolate(s: string, args: ExecuteArgs): string {
  return s.replace(/\{\{\s*([\w.]+)\s*\}\}/g, (_, key) => {
    ...
    if (ns === 'appointment' && prop) {
      return String(
        args.context.appointment?.[
          prop as keyof NonNullable<AutomationContext['appointment']>
        ] ?? ''
      );
    }
    return '';
