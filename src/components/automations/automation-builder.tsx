...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  send_appointment_template: {
    label: 'send_appointment_template',
    icon: FileText,
    border: 'border-l-emerald-500',
  },
};
...
const TRIGGER_OPTIONS: { value: AutomationTriggerType }[] = [
  ...
  { value: 'appointment_reminder_due' },
...
function StepEditor({
  ...
  switch (step.step_type) {
    ...
    case 'create_appointment':
      return (
        <>
          <FieldBlock label="Service ID">
            <Input
              value={(cfg.service_id as string) ?? ''}
              onChange={(e) => set({ service_id: e.target.value })}
              placeholder="Appointment service UUID"
              className="bg-muted text-foreground"
            />
          </FieldBlock>
          <FieldBlock label="Staff scheduling profile ID">
            <Input
              value={(cfg.staff_profile_id as string) ?? ''}
          ...
          <FieldBlock label="Start time">
            <Input
              ...
              placeholder="{{vars.starts_at}} or ISO timestamp"
              className="bg-muted text-foreground"
            />
          </FieldBlock>
          <label className="text-muted-foreground flex items-center gap-2 text-xs">
            <input
              type="checkbox"
              checked={Boolean(cfg.confirm_immediately)}
              onChange={(e) => set({ confirm_immediately: e.target.checked })}
            />
            Confirm immediately
          </label>
        </>
    ...
    case 'offer_appointment_slots':
      return (
        <>
          ...
          <FieldBlock label="Staff profile ID (optional; blank = first available)">
            <Input
              value={(cfg.staff_profile_id as string) ?? ''}
              onChange={(e) => set({ staff_profile_id: e.target.value })}
              className="bg-muted text-foreground"
          ...
          <FieldBlock label="Date">
            <Input
              value={(cfg.date as string) ?? ''}
              onChange={(e) => set({ date: e.target.value })}
              placeholder="YYYY-MM-DD or {{vars.date}}"
              className="bg-muted text-foreground"
            />
          </FieldBlock>
        </>
    ...
    case 'cancel_appointment':
      return (
        <FieldBlock label="Appointment ID (blank uses trigger context)">
          <Input
            value={(cfg.appointment_id as string) ?? ''}
    ...
    case 'reschedule_appointment':
      return (
        <>
          ...
          <FieldBlock label="New start time">
            <Input
              value={(cfg.starts_at as string) ?? ''}
              onChange={(e) => set({ starts_at: e.target.value })}
              placeholder="{{vars.starts_at}}"
              className="bg-muted text-foreground"
            />
          </FieldBlock>
        </>
...
function previewFor(step: BuilderStep): string {
  switch (step.step_type) {
    ...
    case 'create_appointment':
      return (step.step_config.starts_at as string) || 'configure appointment';
    case 'offer_appointment_slots':
      return (step.step_config.date as string) || 'choose a date';
    case 'confirm_appointment':
    case 'cancel_appointment':
    case 'reschedule_appointment':
      return (
        (step.step_config.appointment_id as string) || 'from trigger context'
      );
    default:
      return '';
  }
