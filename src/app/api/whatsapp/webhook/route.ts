...
import { consumeAppointmentAction } from '@/lib/appointments/actions';
...
export async function POST(request: Request) {
  ...
  // Support both Meta format (x-hub-signature-256) and Kapso format (x-webhook-signature)
  const signature =
...
async function processMessage(
  ...
  // ============================================================
  const appointmentAction = interactiveReplyId
    ? await consumeAppointmentAction({
        token: interactiveReplyId,
        accountId,
        contactId: contactRecord.id,
      }).catch((error) => {
        console.error('[appointments] action consume failed:', error);
        return { consumed: false as const };
      })
    : { consumed: false as const };

  if (appointmentAction.consumed) {
    const reply =
      appointmentAction.outcome === 'book'
        ? 'Your appointment is booked and confirmed.'
        : appointmentAction.outcome === 'confirm'
          ? 'Your appointment is confirmed.'
          : appointmentAction.outcome === 'cancel'
            ? 'Your appointment has been cancelled.'
            : appointmentAction.outcome === 'reschedule'
              ? 'Your appointment has been rescheduled.'
              : appointmentAction.outcome === 'unavailable'
                ? 'That time is no longer available. Please choose another slot.'
                : 'That appointment option has expired. Please request fresh options.';
    engineSendText({
      accountId,
      ...
      console.error('[appointments] action acknowledgement failed:', error)
    );
  }

  const flowResult = appointmentAction.consumed
    ? { consumed: true as const, outcome: 'appointment_action' }
