...
export function ContactDetailView({
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    const response = await fetch(`/api/appointments?contact_id=${contactId}`);
    const body = await response.json();
    setAppointments(response.ok ? (body.appointments ?? []) : []);
                ...
                >
                  {loadingAppointments ? (
                    ...
                    <div className="py-8 text-center">
                      <CalendarDays className="text-muted-foreground mx-auto mb-2 size-5" />
                      <p className="text-muted-foreground text-xs">
