...
function validateOne(
  ...
  switch (step.step_type) {
    ...
    case 'create_appointment':
      if (!nonEmpty(c.service_id)) {
        issues.push({
          path: `${path}.service_id`,
          message: 'appointment service is required',
        });
      }
      if (!nonEmpty(c.staff_profile_id)) {
        issues.push({
          path: `${path}.staff_profile_id`,
          message: 'appointment staff is required',
        });
      }
    ...
    case 'offer_appointment_slots':
      ...
      if (!nonEmpty(c.date)) {
        issues.push({ path: `${path}.date`, message: 'slot date is required' });
      }
      break;
    ...
    case 'cancel_appointment':
      // Appointment id may come from appointment trigger context.
      break;
    default:
